...
class AuxilaryVerbs(Rule):
    ...
    def apply(self, tree: LexcialTreeNode, annotations: List[IGTag]):
        ...
        if len(root) == 1:
            ...
            aux_do = [n for n in root[0].children if n.relation == "aux" and n.lemm in ["do", "be"]]
            if len(aux_do) == 1:
                annotations.append(
    ...
    def apply(self, tree: LexcialTreeNode, annotations: List[IGTag]):
        ...
        for c in tree.children:
            if c.relation in ["nsubj", "nsubj:pass"]:
                ...
                annotations.append(
                    IGTag(
                        words=[(cc.id, cc.value) for cc in c.get_all_descendants() if cc != c and cc.relation == "det"],
                        tag_name=IGElement.ATTRIBUTE,
                    )
                )
                annotations.append(
